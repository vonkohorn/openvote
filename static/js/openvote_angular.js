...
function ElectionListCtrl($scope, Election) {
  ...
  $scope.elections = [
    {name:'US Presidential Election 2012', voted:false},
    {name:'Some Congressional Election', voted:false},
    {name:'Another Congressional Election', voted:false}
  ];
...
function AddElectionCtrl($scope, $location, Election) {
  $scope.save = function() {
    Election.save($scope.election, function(election) {
      $location.path('election/edit/' + project._id.$oid);
    });
  }
}
